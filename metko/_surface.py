...
def _surf_dists(mask_gt, mask_pred, spacing_rw):
    """Compute the closest distances from all surface points to the other surface.

    Finds all surface elements "surfels" in the ground truth mask `mask_gt` and
    the predicted mask `mask_pred`, computes their area in (real world units)^2
    and the distance to the closest point on the other surface. It returns two sorted
    lists of distances together with the corresponding surfel areas. If one of the masks
    ...
    Args:
        ...
        mask_pred: 3-dim Numpy array of type bool
            The predicted mask.
        spacing_rw: 3-element list-like structure
            Voxel spacing in x0, x1 and x2 direction.

    Returns:
        A dict with:
        "distances_gt_to_pred": 1-dim numpy array of type float
            The distances in real world units from all ground truth surface elements
            to the predicted surface, sorted from smallest to largest.
        "distances_pred_to_gt": 1-dim numpy array of type float
            The distances in real world units from all predicted surface elements
            to the ground truth surface, sorted from smallest to largest.
        "surfel_areas_gt": 1-dim numpy array of type float
            The area in (real world units)^2 of the ground truth surface elements
            in the same order as distances_gt_to_pred.
        "surfel_areas_pred": 1-dim numpy array of type float
            The area in (real world units)^2 of the predicted surface elements
            in the same order as distances_pred_to_gt.
    """

    # compute the area for all 256 possible surface elements
    # (given a 2x2x2 neighbourhood) according to the spacing_rw
    ...
    for code in range(256):
        ...
        for normal_idx in range(normals.shape[0]):
            # normal vector
            n = np.zeros([3])
            n[0] = normals[normal_idx, 0] * spacing_rw[1] * spacing_rw[2]
            n[1] = normals[normal_idx, 1] * spacing_rw[0] * spacing_rw[2]
            n[2] = normals[normal_idx, 2] * spacing_rw[0] * spacing_rw[1]
...
def surf_overlap_at_tol(input_, target, num_classes, spacing_rw, tolerance_rw,
    ...
    Args:
        ...
        num_classes: int
            Total number of classes.
        spacing_rw: 3-tuple
            Pixel spacing in real world units, one value per each spatial dimension.
        tolerance_rw: float
    ...
    def _soat(surface_distances, tolerance_rw):
        ...
        surfel_areas_pred = surface_distances["surfel_areas_pred"]
        rel_overlap_gt = (
                np.sum(surfel_areas_gt[distances_gt_to_pred <= tolerance_rw]) /
                np.sum(surfel_areas_gt))
        rel_overlap_pred = (
                np.sum(surfel_areas_pred[distances_pred_to_gt <= tolerance_rw]) /
                np.sum(surfel_areas_pred))
...
def surf_dice_at_tol(input_, target, num_classes, spacing_rw, tolerance_rw,
    ...
    def _sdat(surface_distances, tolerance_rw):
        ...
        surfel_areas_gt = surface_distances["surfel_areas_gt"]
        surfel_areas_pred = surface_distances["surfel_areas_pred"]
        overlap_gt = np.sum(surfel_areas_gt[distances_gt_to_pred <= tolerance_rw])
        overlap_pred = np.sum(surfel_areas_pred[distances_pred_to_gt <= tolerance_rw])
